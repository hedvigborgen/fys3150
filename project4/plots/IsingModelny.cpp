...
//   m_ExpEnergy /= Cycle;
//   m_ExpEnergySquared /= Cycle;
//   m_ExpMagneticMoment /= Cycle;
//   m_ExpMagneticMomentSquared /= Cycle;
// }
  ...
  for (int Cycle = 1; Cycle < m_MCCs; Cycle++){
    ...
    for (int n = 1; n <= m_NSpins; n++){
      ...
      if (DeltaEnergy <= 0){
        ...
        totEnergy += DeltaEnergy;
        //cout << "DeltaE = " << DeltaEnergy << " , cycle = " << Cycle << endl;
      }
...
void IsingModel::CalculatingProbability(){
  //vec UniqueElements = unique(m_StoreEnergies);
  //uvec Histogram = hist(m_StoreEnergies, UniqueElements);
  //cout << Histogram << endl;
