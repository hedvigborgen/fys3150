...
for i in range(Ntemps):
    T = T_start + i*h
    values = np.loadtxt(f'../output/{T}.dat')
...
fig.tight_layout()
fig.savefig(f'../plots/exp_energy_parallel{MCCs}.pdf')
...
fig.tight_layout()
fig.savefig(f'../plots/exp_magnetization_parallel{MCCs}.pdf')
...
ax.legend(fontsize=15)
fig.savefig(f'../plots/specific_heat_capacity_parallel{MCCs}.pdf')
...
ax.legend(fontsize=15)
fig.savefig(f'../plots/susceptibility_parallel{MCCs}.pdf')
