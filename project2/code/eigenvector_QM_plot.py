...
fig, ax = plt.subplots()
ax.plot(rho, abs(psi[0])**2, color='#666699', label='$\omega_r = 0.01$')
ax.plot(rho, abs(psi[1])**2, color='#DB7093', label='$\omega_r = 0.5$')
ax.plot(rho, abs(psi[2])**2, color='#BC8F8F', label='$\omega_r = 1.0$')
ax.plot(rho, abs(psi[3])**2, color='#A52A2A', label='$\omega_r = 5.0$')
plt.legend()
ax.set_xlabel(r'$\rho$')
ax.set_ylabel(r'$|\psi(\rho)|^2$')
ax.set_title(r'$|\psi(\rho)|^2$ for matrix of size $n$ = {} with various $\omega_r$'.format(n))
